...
#
>真正的智能，不是来自"**正确**"，而是来自"**错误**"，因为正确是需要人工去标注的，而错误是可以系统自动记载并拦截的。
>
...
公共事实（知识）         私域事实（个体）
      ...
      └──────────┬───────────┘
            ...
            提供给 LLM
```
- 公共事实 → 可以来自公开文献、法规、论文、知识库。
- 私域事实 → 来自用户自己的事实库（企业、医院、个人等）。
- NMP → 不负责创造事实，只负责根据当前目的检索相关事实。
- LLM → 基于检索到的事实进行语言组织、分析和推理。
